...
export default function DrawerForm({
  ...
  // Debug log to check if companies are received
  console.log("📦 DrawerForm - Companies received:", companies);
  console.log("📦 DrawerForm - Companies count:", companies.length);
  ...
  const fetchExistingAttachments = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/attachments?related_type=${entityType}&related_id=${entity.id}`, { credentials: "include" });
          ...
          >
            ...
            {field.name === "lead_status" && isConverted ? (
              ...
              <>
                ...
                {/* ===== SELECT FIELDS RENDERING ===== */}

                {/* 1️⃣ COMPANY ID - Use fetched companies */}
                {field.type === "select" && field.name === "company_id" && (
                  <CustomDropdown
                    field={{
                      ...field,
                ...
                {/* 4️⃣ ALL OTHER SELECTS (excluding owner_id, attendees, company_id) */}
                {field.type === "select" && 
                 field.name !== "owner_id" && 
