...
useEffect(() => {
  ...
  const handleHangUp = useCallback((duration, forceOutcome) => {
    ...
    setCallData({
      connected: entityState?.lead_name || entityState?.ticket_name || entityState?.name,
  ...
  useEffect(() => {
    ...
    if (found) {
      ...
      fetch(`${API_BASE_URL}/tickets/${id}`, {
        ...
        .then(async (res) => {
          console.log(`[TICKET FALLBACK] Status: ${res.status} ${res.statusText}`);
          const text = await res.text();
          try {
            return JSON.parse(text);
          } catch (e) {
            console.error("[TICKET FALLBACK] Failed to parse JSON body:", text);
            return { error: "Invalid JSON response" };
          }
        })
        .then(data => {
          console.log("[TICKET FALLBACK] Data received:", data);
          if (data && !data.error && Object.keys(data).length > 0) {
            setEntityState(data);
          } else {
             console.error("[TICKET FALLBACK] Error or empty data received:", data);
          }
        })
        .catch(err => console.error("[TICKET FALLBACK] Fetch error:", err))
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/calls/initiate`, {
          ...
          body: JSON.stringify({
            ...
            entityName: entityState?.lead_name || entityState?.ticket_name || entityState?.name
          })
        });
        const data = await res.json();
        if (data.sid) {
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.rightDesktop}>
          <RightPanel entityId={entityState.id} entityType="tickets" refreshKey={refreshKey} />
        </div>
      </div>
