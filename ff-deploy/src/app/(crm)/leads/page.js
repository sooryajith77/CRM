...
import { showToast } from "@/services/toastService";
import { formConfig } from "@/config/drawer/tableForm/formConfig";
import useApiEntity from "@/hooks/useEntities"; // ✅ USE THIS HOOK
import PopupMessage from "@/components/ui/PopupMessage/PopupMessage";
import TableSkeleton from "@/components/ui/Skeleton/TableSkeleton";
import ImportModal from "@/components/ui/ImportModal/ImportModal";
import BulkDeleteButton from "@/components/ui/BulkDeleteButton/BulkDeleteButton";
...
export default function LeadsPage() {
  ...
  const { data, loading, createEntity, bulkCreateEntity, updateEntity, deleteEntity, bulkDeleteEntity } = useApiEntity(entity);
  ...
  // ✅ FETCH COMPANIES - USING THE SAME API PATTERN
  useEffect(() => {
    const fetchCompanies = async () => {
      ...
      try {
        ...
        console.log("📡 Companies API status:", res.status);
        ...
        if (res.ok) {
          ...
          if (Array.isArray(data)) {
            ...
            companyOptions = data.data.map(company => ({
              value: company.id || company.company_id,
              label: company.name || company.company_name || `Company ${company.id}`
            }));
          }
          
          console.log("✅ MAPPED COMPANIES:", companyOptions);
        ...
        console.log("📦 Using mock companies:", mockCompanies);
