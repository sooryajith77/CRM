...
useEffect(() => {
  ...
  const handleHangUp = useCallback((duration, forceOutcome) => {
    ...
    setCallData({
      connected: entityState?.lead_name || entityState?.deal_name || `${entityState?.first_name || ""} ${entityState?.last_name || ""}`,
  ...
  useEffect(() => {
    ...
    if (found) {
      ...
      console.log(`[DEAL FALLBACK] Fetching ID: ${id}`);
      fetch(`${API_BASE_URL}/deals/${id}`, {
        credentials: "include"
      })
        .then(async (res) => {
          console.log(`[DEAL FALLBACK] Status: ${res.status} ${res.statusText}`);
          const text = await res.text();
          try {
            return JSON.parse(text);
          } catch (e) {
            console.error("[DEAL FALLBACK] Failed to parse JSON body:", text);
            return { error: "Invalid JSON response" };
          }
        })
        .then(data => {
          console.log("[DEAL FALLBACK] Data received:", data);
          if (data && !data.error && Object.keys(data).length > 0) {
            setEntityState(data);
          } else {
             console.error("[DEAL FALLBACK] Error or empty data received:", data);
          }
        })
        .catch(err => console.error("[DEAL FALLBACK] Fetch error:", err))
  ...
  // 🔥 UNIFIED CONTACT INFO FOR UI
  const contactInfo = {
    email: entityState?.email || entityState?.lead_email,
    phone: entityState?.phone || entityState?.lead_phone
  };
    ...
    if (type === "call") {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/calls/initiate`, {
          ...
          credentials: "include",
          body: JSON.stringify({
            phone: entityState?.lead_phone || entityState?.phone,
            entityName: entityState?.lead_name || entityState?.deal_name || `${entityState?.first_name || ""} ${entityState?.last_name || ""}`
          })
        });
        const data = await res.json();
        if (data.sid) {
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.rightDesktop}>
          <RightPanel entityId={entityState.id} entityType="deals" refreshKey={refreshKey} />
        </div>
      </div>
      ...
      <CallOverlay
        ...
        status={callStatus}
        entityName={entityState?.lead_name || entityState?.deal_name || `${entityState?.first_name || ""} ${entityState?.last_name || ""}`}
        phoneNumber={entityState?.lead_phone || entityState?.phone}
