...
export default function LeadsPage() {
  ...
  const handleExport = () => {
    ...
    let headers = ["company_name", "email", "domain_name", "type", "industry", "phone", "no_of_employees", "annual_revenue", "city", "country", "created_at"];
  ...
  const filteredData = useMemo(() => {
    ...
    if (selectedLeadId) {
      result = result.filter(
        (item) => String(item.lead_id) === String(selectedLeadId)
      );
    }

    return result;
  }, [data, search, filters, searchFields, selectedLeadId]);
  ...
  const checkDisabledRow = (row) => {
    return row.has_leads || row.has_deals || row.has_tickets;
  };
