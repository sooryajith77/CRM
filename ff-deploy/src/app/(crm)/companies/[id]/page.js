...
useEffect(() => {
  ...
  const handleHangUp = useCallback((duration, forceOutcome) => {
    ...
    setCallData({
      connected: entityState?.lead_name || entityState?.company_name || entityState?.name,
  ...
  useEffect(() => {
    ...
    if (found) {
      ...
      console.log(`[COMPANY FALLBACK] Fetching ID: ${id}`);
      fetch(`${API_BASE_URL}/companies/${id}`, {
        credentials: "include"
      })
        .then(async (res) => {
          console.log(`[COMPANY FALLBACK] Status: ${res.status} ${res.statusText}`);
          const text = await res.text();
          try {
            return JSON.parse(text);
          } catch (e) {
            console.error("[COMPANY FALLBACK] Failed to parse JSON body:", text);
            return { error: "Invalid JSON response" };
          }
        })
        .then(data => {
          console.log("[COMPANY FALLBACK] Data received:", data);
          if (data && !data.error && Object.keys(data).length > 0) {
            setEntityState(data);
          } else {
             console.error("[COMPANY FALLBACK] Error or empty data received:", data);
          }
        })
        .catch(err => console.error("[COMPANY FALLBACK] Fetch error:", err))
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/calls/initiate`, {
          ...
          body: JSON.stringify({
            ...
            entityName: entityState?.lead_name || entityState?.company_name || entityState?.name
          })
        });
        const data = await res.json();
        if (data.sid) {
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.rightDesktop}>
          <RightPanel entityId={entityState.id} entityType="companies" refreshKey={refreshKey} />
        </div>
      </div>
